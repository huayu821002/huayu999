...
export default function EmailTemplatesPage() {
  ...
  // Fetch templates after auth is confirmed
  const fetchTemplates = useCallback(async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        const first = data.data['welcome']
        if (first) setEditForm(first)
        setTemplatesLoaded(true)
      }
    } catch (err) {
      console.error('[EmailTemplates] fetch error:', err)
    }
  }, [token])
  ...
  }, [authChecked, token, fetchTemplates])
  ...
  const sendTestEmail = async () => {
    if (!testEmail) { setStatusMsg('Enter a test email'); return }
    setIsSaving(true)
    setStatusMsg(null)
    try {
  ...
  // Not yet checked auth — show loading
  if (!authChecked) {
    return (
  ...
  // Admin but templates not loaded yet — show loading
  if (authChecked && !templatesLoaded) {
    return (
      <div className="min-h-screen bg-joy-gray-50 flex items-center justify-center">
        <div className="animate-spin w-8 h-8 border-4 border-joy-orange border-t-transparent rounded-full" />
      </div>
